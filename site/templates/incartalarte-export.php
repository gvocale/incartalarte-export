...
foreach ($posts->limit(1000) as $post) {
	...
	$metas = $postmeta->filterBy("post_id", $post->ID());
	...
	$post = $posts->filterBy("ID", $product["thumbnail_id"]);
	...
	$metas = $postmeta->filterBy("post_id", $product["thumbnail_id"]);
	foreach ($metas as $meta) {
	...
	$categories = $term_relationships->filterBy("object_id", $post->ID());
